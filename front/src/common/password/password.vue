...
export default {
  ...
  methods: {
    ...
    click() {
      this.pusform.oldpass = Md5(this.form.password);
      this.pusform.newpass = Md5(this.form.passwordAgain);
      console.log(this.pusform);
      this.$refs["form"].validate(async (valid) => {
        if (valid) {
          const { data: res } = await this.axios.put("/user/changePass", this.pusform);
          if (res.code === 200) {
