  ...
  <div>
    <el-card class="cardStyle" v-loading="loading">
      ...
      <el-collapse-transition>
        <div
          v-if="show"
          style="background-color: white; height: 100px; box-sizing: border-box"
        >
          <el-form ref="form" :model="form" label-width="80px">
            <el-row v-if="userInfo.type === 2">
              <el-col :span="8">
...
export default {
      ...
      dataColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            if (this.batch || this.showInput === params.row.studentId) {
              return h("el-input", {
                ...
                on: {
                  input(value) {
                    ...
                    params.row.scoreByUser =
                      value > params.row.score ? params.row.score : value;
                  },
                },
              });
            } else {
              ...
              let scoreValue =
                params.row.scoreByUser === null ? "未录入" : params.row.scoreByUser;
        ...
        },
        {
          ...
          render(h, params) {
            ...
            let creditsValue =
              params.row.creditsByUser === null ? 0 : params.row.creditsByUser;
  ...
  methods: {
    ...
    clickMethod(obj) {
      ...
      this.axios.get(baseurl + "/getScoreList", { params: obj }).then((response) => {
    ...
    click(page) {
      ...
      let obj = {
        ...
        id: userInfo.type === 2 ? userInfo.id : "",
        type: userInfo.type,
        year: this.form.year,
        term: this.form.term,
      };
    ...
    addEntry() {
      ...
      if (this.selection.length > 0) {
        if (flag) {
          ...
          this.axios
            .post(baseurl + "/score", this.selection)
            .then(() => {
    ...
    exportMethod() {
      ...
      let obj = {
        ...
        id: userInfo.type !== 2 ? userInfo.id : "",
        courseName: this.form.courseName,
        studentName: userInfo.type === 2 ? userInfo.id : "",
        type: userInfo.type,
      };
      var baseurl = "";
      if (this.userInfo.type === 2) {
      ...
      this.axios.get(baseurl + "/getScoreList", { params: obj }).then((response) => {
        const { data: res } = response;
        const list = res.data.items; //把data里的tableData存到list
        list.map((data) => {
          if (data.type === 1) {
            data.type = "必修";
          } else if (data.type === 2) {
    ...
    exportExcel(list, userInfo) {
      require.ensure([], () => {
        ...
        const title =
          userInfo.type === 2 ? `${userInfo.realName}成绩列表` : "学生成绩列表";
    ...
    getProfessionByAdmin() {
      ...
      this.axios
        .get(baseurl + "/getProfessionInfoByAdmin")
        .then((response) => {
          const { data: res } = response;
    ...
    getProfessionByTeacher() {
      var url = "";
      if (this.userInfo.type === 0) {
        url = "/admin/teach/getProfessionInfoByTeacher/" + this.userInfo.id;
      } else if (this.userInfo.type === 1) {
        url = "/teacher/getProfessionInfoByTeacher";
      }
      this.axios
        .get(url)
        .then((response) => {
  ...
  mounted() {
    ...
    if (this.userInfo.type !== 2) this.click(this.searchValue);
  },
  created() {
