<template>
  ...
  >
    ...
    <VmBaseTable
      ...
      :tableHigh="tableHigh"
    ></VmBaseTable>
    <div slot="footer" v-if="userInfo.type === 0">
  ...
  methods: {
    ...
    getClass() {
      ...
      this.axios
        .get(baseUrl + "/getTree")
        .then((response) => {
          this.classArr = response.data.data[0].children;
        })
        .catch((error) => {
          this.$message.error(
            {
    ...
    getCourse(profession, term) {
      ...
      this.axios.get("/admin/course/getCourseByMap", { params: obj }).then((response) => {
    ...
    getInfo(id) {
      ...
      this.axios.get(baseUrl + "/getCourseListById/" + id).then((response) => {
    ...
    clickMethod() {
      this.axios
        .post("/admin/teach", this.dataTable)
        .then(() => {
          this.doneNum = 0;
    ...
    deleteSingle(data) {
      if (data.id === -1) {
        ...
        this.axios.delete("/admin/teach/" + data.id).then(() => {
          this.$message.success({
            message: "任课信息删除成功",
          });
